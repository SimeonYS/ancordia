...
class AncdordiaPipeline:

    # Database setup
    conn = sqlite3.connect('ancordia.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ancordia`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ancordia WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ancordia (date, title, link, content)"
